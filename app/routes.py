...
from app.forms import FillUpForm, VehicleSettingsForm, LoginForm, RegisterForm
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if existing:
            flash(_('An account with this license number already exists.'), 'error')
        else:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        flash(_('Invalid license number or password.'), 'error')
...
def add_fillup():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if efficiency:
                flash(_('Fill-up added successfully! Fuel efficiency: %(efficiency).1f L/100km', efficiency=efficiency), 'success')
            else:
                flash(_('Fill-up added successfully! Add more entries to see fuel efficiency.'), 'success')
            
            return redirect(url_for('main.index'))
            
        except Exception as e:
            db.session.rollback()
            flash(_('Error adding fill-up: %(error)s', error=str(e)), 'error')
...
def range_predictor():
    ...
    if request.method == 'POST':
        try:
            ...
            if current_fuel <= 0:
                flash(_('Please enter a valid fuel amount greater than 0.'), 'error')
            elif current_fuel > 200:
                flash(_('Fuel amount seems unusually high. Please check your input.'), 'error')
            else:
                # Calculate predicted range: (fuel ÷ efficiency) × 100
                predicted_range = (current_fuel / average_efficiency) * 100
                
                flash(_('Range calculated successfully! Based on your average efficiency of %(efficiency).1f L/100km.', efficiency=average_efficiency), 'success')
                
        except (ValueError, TypeError):
            flash(_('Please enter a valid number for fuel amount.'), 'error')
...
def delete_fillup(id):
    ...
    try:
        ...
        db.session.commit()
        flash(_('Fill-up from %(date)s deleted successfully.', date=fillup.date.strftime("%Y-%m-%d")), 'success')
    except Exception as e:
        db.session.rollback()
        flash(_('Error deleting fill-up: %(error)s', error=str(e)), 'error')
...
def vehicle_settings():
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.commit()
            flash(_('Vehicle settings updated successfully!'), 'success')
            return redirect(url_for('main.index'))
            
        except Exception as e:
            db.session.rollback()
            flash(_('Error updating vehicle settings: %(error)s', error=str(e)), 'error')
